...
class Dataset:
    def __init__(self, conf):
        ...
        for im_name in self.images_lis:
            arr = _safe_imread(im_name)
            if arr is None:
                raise RuntimeError(f"Failed to read image: {im_name}")
            imgs.append(arr)
        self.images_np = np.stack(imgs, axis=0) / 255.0

        # Load masks
        self.masks_lis = sorted(glob(os.path.join(self.data_dir, 'mask/*')))
        masks = []
        if len(self.masks_lis) == len(self.images_lis):
            for mk_name, im_name in zip(self.masks_lis, self.images_lis):
                mk = _safe_imread(mk_name)
                if mk is None:
                    base = _safe_imread(im_name)
                    mk = _all_white_mask_like(base)
                elif mk.ndim == 2:
                    mk = np.repeat(mk[..., None], 3, axis=2)
                masks.append(mk)
        else:
            for im_name in self.images_lis:
                base = _safe_imread(im_name)
                masks.append(_all_white_mask_like(base))
        self.masks_np = np.stack(masks, axis=0) / 255.0

        # Camera parameters
        self.world_mats_np = [camera_dict[f'world_mat_{idx}'].astype(np.float32)
                              for idx in range(self.n_images)]
        self.scale_mats_np = [camera_dict[f'scale_mat_{idx}'].astype(np.float32)
                              for idx in range(self.n_images)]
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        rays_o = self.pose_all[img_idx, :3, 3].expand(rays_v.shape)
